...
- `data/train_embeddings.npy` — Cached DistilBERT embeddings, training set (included)  
...
The raw file (`Software.jsonl`, ~1.87 GB) is **not included** in this repository and is not needed to reproduce results. The pre-processed subsample (`software_sample.csv`) and cached embeddings are included and loaded automatically by the notebook.
...
```bash
pip install scikit-learn transformers torch nltk pandas numpy matplotlib seaborn
```
...
- DistilBERT embeddings are pre-cached in `data/` — no model download required to reproduce results.
- If regenerating embeddings from scratch, a HuggingFace account may be required. Create a free Read token at https://huggingface.co/settings/tokens and run `from huggingface_hub import login; login()` before the embedding extraction cell.
